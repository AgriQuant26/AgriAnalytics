...
def run_pipeline(spark):
    input_file = "abfss://datalake-agrisynapse@agrisynapse.dfs.core.windows.net/silver/cleaned_crop_data_final.csv"
    #create output folder
    output_base = "abfss://datalake-agrisynapse@agrisynapse.dfs.core.windows.net/gold/fact_and_dimensions/"
    
    df = spark.read.option("header", True).option("inferschema",True).csv(input_file).toPandas()
    #standardise column names
    df.columns = (df.columns.str.strip()
                    .str.lower()
                    .str.replace(" ","_"))
    #convert expected numeric columns
    numeric_expected = ["crop_year", "area", "production", "fertilizer", "pesticide", "annual_rainfall"]
    ...
    # writing output to ADLS
    spark.createDataFrame(dim_location).coalesce(1).write.mode("overwrite").option("header", True).csv(output_base + "dim_location")
    print("Saved dim_location")

    spark.createDataFrame(dim_crop).coalesce(1).write.mode("overwrite").option("header", True).csv(output_base + "dim_crop")
    print("Saved dim_crop")

    spark.createDataFrame(dim_year).coalesce(1).write.mode("overwrite").option("header", True).csv(output_base + "dim_year")
    print("Saved dim_year")

    spark.createDataFrame(fact_production).coalesce(1).write.mode("overwrite").option("header", True).csv(output_base + "fact_production")
    print("Saved fact_production")
