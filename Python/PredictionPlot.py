...
print("2021 data generated successfully using Linear Regression in csv!")

import matplotlib.pyplot as plt

# 1. Plot the actual data
plt.figure(figsize=(12, 6))
plt.scatter(df['Order Date'], df['Sales'], color='blue', label='Actual Sales (2016-2020)', alpha=0.5)

# 2. Plot the predicted data
plt.scatter(pd.to_datetime(df_2021['Order Date']), df_2021['Sales'], color='red', label='Predicted Sales (2021)', alpha=0.5)

# 3. Add labels and title
plt.title('Sales Forecast: Linear Regression Trend')
plt.xlabel('Year')
plt.ylabel('Sales ($)')
plt.legend()
plt.grid(True)

# 4. Save the plot to your folder
